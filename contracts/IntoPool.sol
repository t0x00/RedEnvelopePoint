...
contract IntoPool is AdminRoleUpgrade, Initializable {
    ...
    function stakeReceive() external check{
        _getStakeReceive(msg.sender);
    }

    function getStakeReceive(address _addr) public onlyAdmin{
        _getStakeReceive(_addr);
    }

    // 领取收益
    function _getStakeReceive(address _addr) internal {
        require(isMining[_addr], "already receive");
        
        StakeDetail[] storage records = boxStakeMap[_addr];
       
        // 当前时间加1天
        uint256 nextDayTimestamp = block.timestamp.getNextDayTimestamp();
        uint256 mystakeStartTime = stakeStartTime[_addr].getTodayTimestamp();
                ...
                if(record.endTime < nextDayTimestamp){
                    ...
                    if(record.startTime < 1672329600){
                        ...
                        if(checkBool(record.name, "F")){
                            activePoint.updateFavoriteAndPoint(_addr, 0,0, record.activePoint, false, 10);
                        }else{
        ...
        if(expirePoint > 0 || oldExpirePoint > 0){
            // 同步数据
            activePoint.initSetWithFirst(_addr);
            if(expirePoint > 0){
                activePoint.updateFavoriteAndPoint(_addr, 0,0, expirePoint, false, 30);
            }
            if(oldExpirePoint > 0){
                activePoint.updateFavoriteAndPoint(_addr, 0,0, oldExpirePoint, false, 60);
            }
        ...
        if(ownerbalances > 0){
            updateOwnerBalances(_addr, ownerbalances, "");
            updateParentBalances(_addr, ownerbalances, "");
        }

        isMining[_addr] = false;
    }
    ...
    function getExpectToken(address _addr,uint256 _token, uint256 _day) public view returns(uint256){
        ...
        uint256 needtime = endtime - stakeStartTime[_addr];
    ...
    // function setMing(address addr) external {
    //     isMining[addr] = false;
    //     stakeStartTime[addr] = 0;
    // }
